...
class BillRemoteDataSourceImpl implements BillRemoteDataSource {
  ...
  Future<List<GetBillsModel>> getBill(String consumerNumber) async {
    ...
    AppLogger.info(
      'Fetching bill for consumer: $consumerNumber',
      tag: 'API_GET_BILL',
    );
    AppLogger.info(
      'Request URL: ${URLS.getBillUrl} | Body: $bodySent',
      tag: 'API_GET_BILL',
    );
    ...
    if (response.statusCode == 200 && response.body.isNotEmpty) {
      ...
      SnackbarService.showError('Invalid Consumer Number', '');
      AppLogger.error(
        'Failed to fetch bills. Status: ${response.statusCode}',
        tag: 'API_GET_BILL',
      );
  ...
  Future<CreateBillModel> createBill({
    ...
    AppLogger.info(
      'Creating bill for consumer: $consumerNumber',
      tag: 'API_CREATE_BILL',
    );
    AppLogger.info(
      'Request URL: ${URLS.createBillUrl} | Body: ${jsonEncode(bodySent)}',
      tag: 'API_CREATE_BILL',
    );
    ...
    if (response.statusCode == 200 && response.body.isNotEmpty) {
      ...
      AppLogger.error(
        'Server Error while creating bill: ${response.statusCode}',
        tag: 'API_CREATE_BILL',
      );
